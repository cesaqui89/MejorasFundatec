var assignmentsManagement = {
    ...
    fnAdditionInitializer: function () {
     ...
     fnEditionInitializer: function () {
        ...
        $(assignmentsManagement.actions.fnGetAssignmentForEdition());
    ...
    actions: {
        ...
        fnRedirectToIndex: function(action){
         ...
         fnGetAssignmentForEdition: function(){
             var urlParams = new URLSearchParams(window.location.search);
             var IdAsignacion = urlParams.get('IdAsignacion');
             
            var proccessCallback = function (result)
            {
               $(assignmentsManagement.actions.fnPopulateAssignmentForEdition(result));
            };
            //llamamos la funcion ajax
            var parameters = {'action': "getAssignmentById", 'IdAsignacion': IdAsignacion};
            executeAjax('index.php', parameters, proccessCallback);
         },
         fnPopulateAssignmentForEdition: function(result){
             var encabezado = result[0];            
//            $(assignmentsManagement.controlsId.txtAmount).val(encabezado.Monto).trigger('mask');
